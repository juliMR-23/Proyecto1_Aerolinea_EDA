...
public class MainPageTripulanteCabinaViewController implements Initializable {

    @FXML private Label  lblSubtitulo;
    @FXML private Label  lblTotalVuelosMes;
    @FXML private Label  lblHorasAcumuladas;
    @FXML private Label  lblIdiomas;
    @FXML private Label  lblSalario;
    @FXML private Label  lblMesAnio;
    @FXML private Label  lblProximoVuelo;
    ...
    public void setTripulante(TripulanteCabina tripulante) {
        ...
        lblSubtitulo.setText("Bienvenido, " + tripulante.getNombre() + "  —  Tripulante de cabina");
        lblHorasAcumuladas.setText((int) tripulante.getHorasVueloAcumuladas() + "h");
        lblIdiomas.setText(String.valueOf(tripulante.listIdiomas().length));
        lblSalario.setText("COP " + nf.format((long) tripulante.calcularSalario()));
    ...
    private void cargarProximoVuelo() {
        Vuelo proximo = buscarProximoVuelo();
        if (proximo == null) {
            lblProximoVuelo.setText("No tienes vuelos próximos asignados.");
        } else {
            lblProximoVuelo.setText(
                proximo.getOrigen().getNombre() +
                "  →  " +
                proximo.getDestino().getNombre() +
                "   |   " +
                proximo.getFechaHoraSalida().format(DT_FMT)
            );
        }
    }
    ...
    private void cargarInformeMensual() {
        ...
        Vuelo[]       vuelos     = tripulanteLogueado.getVuelosAsignados();
        LocalDateTime ahora      = LocalDateTime.now();
        int           mesActual  = ahora.getMonthValue();
        int           anioActual = ahora.getYear();
        int           contMes    = 0;

        for (int i = 0; i < tripulanteLogueado.getCantidadVuelos(); i++) {
            Vuelo v = vuelos[i];
            if (v == null) continue;
